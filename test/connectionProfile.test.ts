...
suite('Connection Profile tests', () => {
    ...
    test('CreateProfile should ask questions in correct order', done => {
        ...
        let questionNames: string[] = [
            ...
            LocalizedConstants.portPrompt,       // Port
    ...
    test('Port number should not be applied to server name when connection credentials are transformed into details', () => {
        ...
        assert.strictEqual(details.options['host'], 'my-server');
        assert.strictEqual(details.options['port'], 1234);
    });
